...
model_initialize <- function(yraw, pp, hh,
  ...
  beta_update_expectation <- array(0, dim = c(tt, zz_rows_number))
  ...
  yy_predict_variance <- yy_cov + zz[1:dd, ] %*% beta_update_variance %*%
    t(zz[1:dd, ])
  yy_update_variance <- kk * yy_cov

  list(
    dd = dd, hh = hh, pp = pp, tt = tt, yy = yy, yraw = yraw, zz = zz,
    zz_cols_number = zz_cols_number, zz_rows_number = zz_rows_number,
